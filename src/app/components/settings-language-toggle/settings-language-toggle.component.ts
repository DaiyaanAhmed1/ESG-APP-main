...
import { SettingsTranslationService } from '../../services/settings-translation.service';
import { Subscription } from 'rxjs';

@Component({
  selector: 'app-settings-language-toggle',
  standalone: true,
  ...
  styles: [`
    ...
    .settings-language-toggle:focus {
      outline: none;
      box-shadow: 0 0 0 2px rgba(255, 255, 255, 0.3);
    }

    .settings-language-toggle .icon {
      font-size: 1rem;
    }

    /* RTL support */
    .settings-language-toggle.rtl .icon {
      margin-left: 0.5rem;
      margin-right: 0;
    }
...
export class SettingsLanguageToggleComponent implements OnInit, OnDestroy {
  currentLanguage = 'en';
  isRTL = false;
  private subscription?: Subscription;

  constructor(private translationService: SettingsTranslationService) {}
  ...
  ngOnInit() {
    ...
    setTimeout(() => {
      if (!this.translationService.isTranslationsLoaded()) {
        console.warn('Settings translations not loaded yet');
      }
    }, 1000);
  }
