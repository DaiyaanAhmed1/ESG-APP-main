...
export class CommunicationHubTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Communication Hub translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Communication Hub translations from JSON files:', error);
  ...
  private loadFallbackTranslations() {
    this.translations = {
      en: {
        COMMUNICATION_HUB: {
          TITLE: "Communication Hub Dashboard",
          WELCOME: "Welcome to Communication Hub",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        COMMUNICATION_HUB: {
          TITLE: "لوحة تحكم Communication Hub",
          WELCOME: "مرحباً بك في Communication Hub",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Communication Hub translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Communication Hub translation error for key: ${key}`, error);
      return key;
    }
  }
