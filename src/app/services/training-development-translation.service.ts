...
export class TrainingDevelopmentTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Training Development translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Training Development translations from JSON files:', error);
    ...
    this.translations = {
      en: {
        TRAINING_DEVELOPMENT: {
          ...
          WELCOME: "Welcome to Training Development",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        TRAINING_DEVELOPMENT: {
          TITLE: "لوحة تحكم Training Development",
          WELCOME: "مرحباً بك في Training Development",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Training Development translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Training Development translation error for key: ${key}`, error);
      return key;
    }
  }
