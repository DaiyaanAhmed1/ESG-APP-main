...
export class AiTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('AI translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load AI translations from JSON files:', error);
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`AI translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`AI translation error for key: ${key}`, error);
      return key;
    }
  }
