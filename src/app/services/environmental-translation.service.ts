...
export class EnvironmentalTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Environmental translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Environmental translations from JSON files:', error);
    ...
    this.translations = {
      en: {
        ENVIRONMENTAL: {
          ...
          WELCOME: "Welcome to Environmental",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        ENVIRONMENTAL: {
          TITLE: "لوحة تحكم Environmental",
          WELCOME: "مرحباً بك في Environmental",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Environmental translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Environmental translation error for key: ${key}`, error);
      return key;
    }
  }
