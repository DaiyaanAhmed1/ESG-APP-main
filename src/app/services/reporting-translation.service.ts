...
export class ReportingTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Reporting translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Reporting translations from JSON files:', error);
  ...
  private loadFallbackTranslations() {
    this.translations = {
      ...
      ar: {
        REPORTING: {
          TITLE: "لوحة تحكم Reporting",
          WELCOME: "مرحباً بك في Reporting",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Reporting translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Reporting translation error for key: ${key}`, error);
      return key;
    }
  }
