...
export class ResourceManagementTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Resource Management translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Resource Management translations from JSON files:', error);
    ...
    this.translations = {
      en: {
        RESOURCE_MANAGEMENT: {
          ...
          WELCOME: "Welcome to Resource Management",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        RESOURCE_MANAGEMENT: {
          TITLE: "لوحة تحكم Resource Management",
          WELCOME: "مرحباً بك في Resource Management",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Resource Management translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Resource Management translation error for key: ${key}`, error);
      return key;
    }
  }
