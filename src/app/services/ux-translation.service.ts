...
export class UxTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('UX translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load UX translations from JSON files:', error);
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`UX translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`UX translation error for key: ${key}`, error);
      return key;
    }
  }
