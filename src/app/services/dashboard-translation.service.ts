...
export class DashboardTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Dashboard translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Dashboard translations from JSON files:', error);
    ...
    this.translations = {
      ...
      ar: {
        DASHBOARD: {
          ...
          WELCOME: "مرحباً بك في Dashboard",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Dashboard translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Dashboard translation error for key: ${key}`, error);
      return key;
    }
  }
