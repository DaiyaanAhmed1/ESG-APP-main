...
export class LocalizationTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Localization translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Localization translations from JSON files:', error);
  ...
  private loadFallbackTranslations() {
    this.translations = {
      ...
      ar: {
        LOCALIZATION: {
          TITLE: "لوحة تحكم Localization",
          WELCOME: "مرحباً بك في Localization",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Localization translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Localization translation error for key: ${key}`, error);
      return key;
    }
  }
