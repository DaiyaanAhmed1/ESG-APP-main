...
export class EnvironmentalTrainingTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Environmental Training translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Environmental Training translations from JSON files:', error);
    ...
    this.translations = {
      en: {
        ENVIRONMENTAL_TRAINING: {
          TITLE: "Environmental Training Dashboard",
          WELCOME: "Welcome to Environmental Training",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        ENVIRONMENTAL_TRAINING: {
          TITLE: "لوحة تحكم Environmental Training",
          WELCOME: "مرحباً بك في Environmental Training",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Environmental Training translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Environmental Training translation error for key: ${key}`, error);
      return key;
    }
  }
