...
export class SecurityTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Security translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Security translations from JSON files:', error);
  ...
  private loadFallbackTranslations() {
    this.translations = {
      ...
      ar: {
        SECURITY: {
          TITLE: "لوحة تحكم Security",
          WELCOME: "مرحباً بك في Security",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Security translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Security translation error for key: ${key}`, error);
      return key;
    }
  }
