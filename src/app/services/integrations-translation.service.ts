...
export class IntegrationsTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Integrations translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Integrations translations from JSON files:', error);
    ...
    this.translations = {
      en: {
        INTEGRATIONS: {
          ...
          WELCOME: "Welcome to Integrations",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        INTEGRATIONS: {
          TITLE: "لوحة تحكم Integrations",
          WELCOME: "مرحباً بك في Integrations",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Integrations translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Integrations translation error for key: ${key}`, error);
      return key;
    }
  }
