...
export class ReportAnalyticsTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Report Analytics translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Report Analytics translations from JSON files:', error);
    ...
    this.translations = {
      en: {
        REPORT_ANALYTICS: {
          ...
          WELCOME: "Welcome to Report Analytics",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        REPORT_ANALYTICS: {
          TITLE: "لوحة تحكم Report Analytics",
          WELCOME: "مرحباً بك في Report Analytics",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Report Analytics translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Report Analytics translation error for key: ${key}`, error);
      return key;
    }
  }
