...
export class ManageTeamTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Manage Team translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Manage Team translations from JSON files:', error);
  ...
  private loadFallbackTranslations() {
    this.translations = {
      en: {
        MANAGE_TEAM: {
          TITLE: "Manage Team Dashboard",
          WELCOME: "Welcome to Manage Team",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        MANAGE_TEAM: {
          TITLE: "لوحة تحكم Manage Team",
          WELCOME: "مرحباً بك في Manage Team",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Manage Team translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Manage Team translation error for key: ${key}`, error);
      return key;
    }
  }
