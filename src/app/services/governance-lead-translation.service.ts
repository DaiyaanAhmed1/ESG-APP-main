...
export class GovernanceLeadTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Governance Lead translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Governance Lead translations from JSON files:', error);
  ...
  private loadFallbackTranslations() {
    this.translations = {
      en: {
        GOVERNANCE_LEAD: {
          TITLE: "Governance Lead Dashboard",
          WELCOME: "Welcome to Governance Lead",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        GOVERNANCE_LEAD: {
          TITLE: "لوحة تحكم Governance Lead",
          WELCOME: "مرحباً بك في Governance Lead",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Governance Lead translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Governance Lead translation error for key: ${key}`, error);
      return key;
    }
  }
