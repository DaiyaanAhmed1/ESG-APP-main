...
export class EsgSpecialistTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('ESG Specialist translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load ESG Specialist translations from JSON files:', error);
  ...
  private loadFallbackTranslations() {
    this.translations = {
      en: {
        ESG_SPECIALIST: {
          TITLE: "ESG Specialist Dashboard",
          WELCOME: "Welcome to ESG Specialist",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        ESG_SPECIALIST: {
          TITLE: "لوحة تحكم ESG Specialist",
          WELCOME: "مرحباً بك في ESG Specialist",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`ESG Specialist translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`ESG Specialist translation error for key: ${key}`, error);
      return key;
    }
  }
