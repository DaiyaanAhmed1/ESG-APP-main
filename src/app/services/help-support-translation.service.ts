...
export class HelpSupportTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Help Support translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Help Support translations from JSON files:', error);
    ...
    this.translations = {
      en: {
        HELP_SUPPORT: {
          ...
          WELCOME: "Welcome to Help Support",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        HELP_SUPPORT: {
          TITLE: "لوحة تحكم Help Support",
          WELCOME: "مرحباً بك في Help Support",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Help Support translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Help Support translation error for key: ${key}`, error);
      return key;
    }
  }
