...
export class ComplianceTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Compliance translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Compliance translations from JSON files:', error);
  ...
  private loadFallbackTranslations() {
    this.translations = {
      ...
      ar: {
        COMPLIANCE: {
          TITLE: "لوحة تحكم Compliance",
          WELCOME: "مرحباً بك في Compliance",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Compliance translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Compliance translation error for key: ${key}`, error);
      return key;
    }
  }
