...
export class StakeholderEngagementTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Stakeholder Engagement translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Stakeholder Engagement translations from JSON files:', error);
    ...
    this.translations = {
      en: {
        STAKEHOLDER_ENGAGEMENT: {
          ...
          WELCOME: "Welcome to Stakeholder Engagement",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        STAKEHOLDER_ENGAGEMENT: {
          TITLE: "لوحة تحكم Stakeholder Engagement",
          WELCOME: "مرحباً بك في Stakeholder Engagement",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Stakeholder Engagement translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Stakeholder Engagement translation error for key: ${key}`, error);
      return key;
    }
  }
