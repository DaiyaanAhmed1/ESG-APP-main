...
export class MaterialityTranslationService {
  ...
  private async loadTranslations() {
    try {
      ...
      if (enTranslations && arTranslations) {
        ...
        console.log('Materiality translations loaded from JSON files:', this.translations);
      }
    } catch (error) {
      console.warn('Failed to load Materiality translations from JSON files:', error);
    ...
    this.translations = {
      en: {
        MATERIALITY: {
          ...
          WELCOME: "Welcome to Materiality",
          OVERVIEW: "Overview",
          METRICS: "Key Metrics",
      ...
      ar: {
        MATERIALITY: {
          TITLE: "لوحة تحكم Materiality",
          WELCOME: "مرحباً بك في Materiality",
          OVERVIEW: "نظرة عامة",
  ...
  translate(key: string): string {
    try {
      ...
      if (!value) {
        console.warn(`Materiality translation not found for key: ${key} in language: ${this.currentLanguage}`);
        return key;
      }
      
      return value;
    } catch (error) {
      console.error(`Materiality translation error for key: ${key}`, error);
      return key;
    }
  }
